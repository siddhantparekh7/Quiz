...
function handleQuestions() {
  //function to shuffle and push 10 questions to questions array
...
function initiateQuestions(){
  console.log("INside");
  let questionData = [];
  var questionsPhp = JSON.parse(document.getElementById("questions").innerHTML);
  
  let key = 1;
  questionsPhp.forEach((question,index)=>{
    if(index == 0){
      let obj = {}
      if(parseInt(question.is_right_choice)){
        obj = {
          questionId : question.q_id,
          question : question.question,
          option1 : question.choice,
    ...
    key=2;
      ...
      let foundIndex = questionData.findIndex(e=>e.questionId == question.q_id);
      questionData[foundIndex][`option${key}`] = question.choice;
      if(parseInt(question.is_right_choice)){
        questionData[foundIndex]["correctOption"] = `option${key}`;
      }
      key++;
    }
  })
...
//fuction to generate random code
...
function getCode(btn) {
  ...
  document.getElementById("code").innerHTML =
    "<center><label style='font-size:35px;'>" + code + "</label></center>";
